...
// GET /analytics/avg-pages-per-session-per-user?start=&end=&limit=
/**
 * @openapi
 * /analytics/avg-pages-per-session-per-user:
 *   get:
 *     tags:
 *       - Analytics
 *     summary: Average pages per session grouped by user
 *     description: |
 *       Aggregates reading sessions to compute the average number of pages per session for each user.
 *       Supports optional date range filtering by session start time.
 *     parameters:
 *       - in: query
 *         name: start
 ...
 *         description: Internal server error
 */
router.get('/avg-pages-per-session-per-user', async (req, res) => {
  try {
    await connectMongo();
    const { start, end } = req.query;
    const limit = Math.min(parseInt(req.query.limit ?? '100', 10), 1000);
      ...
      Object.keys(dateMatch(start, end)).length ? { $match: dateMatch(start, end) } : null,
      {
        $addFields: {
          pagesCount: {
            $cond: [
              { $isArray: '$pagesRead' },
              { $size: { $ifNull: ['$pagesRead', []] } },
              { $toInt: { $ifNull: ['$pages_read', 0] } }
            ]
          }
        }
      },
...
// GET /analytics/avg-pages-per-session-per-book?start=&end=&limit=
/**
 * @openapi
 * /analytics/avg-pages-per-session-per-book:
 *   get:
 *     tags:
 *       - Analytics
 *     summary: Average pages per session grouped by book
 *     description: |
 *       Aggregates reading sessions to compute the average number of pages per session for each book.
 *       Supports optional date range filtering by session start time.
 *     parameters:
 *       - in: query
 *         name: start
 ...
 *         description: Internal server error
 */
router.get('/avg-pages-per-session-per-book', async (req, res) => {
  try {
    await connectMongo();
    const { start, end } = req.query;
    const limit = Math.min(parseInt(req.query.limit ?? '100', 10), 1000);
...
// GET /analytics/device-usage?start=&end=&limit=
/**
 * @openapi
 ...
 *     tags:
 *       - Analytics
 *     summary: Device usage metrics
 *     description: |
 *       Groups reading sessions by device to compute counts and averages.
 *       Device values are normalized to lowercase.
 *       Sessions without an endTime are treated as ongoing and measured up to the time of the query.
 *     parameters:
 *       - in: query
 *         name: start
 ...
 *                     - device: ios
 *                       sessions: 120
 *                       avgSessionMinutes: 22.4
 *                       avgPages: 18.2
 *                     - device: android
 *                       sessions: 95
 *                       avgSessionMinutes: 20.15
 *                       avgPages: 16.75
 *                     - device: web
 *                       sessions: 60
 *                       avgSessionMinutes: 18.05
 *                       avgPages: 12.9
 *       500:
 *         description: Internal server error
 */
router.get('/device-usage', async (req, res) => {
  try {
    ...
    const pipeline = [
      ...
      {
        $project: {
          device: { $toLower: { $ifNull: ['$device', 'unknown'] } },
          durationMs: {
            $max: [
              0,
              { $subtract: [ { $ifNull: ['$endTime', '$$NOW'] }, '$startTime' ] }
      ...
      },
      {
        $group: {
          _id: '$device',
          sessions: { $sum: 1 },
          avgSessionMinutes: { $avg: { $divide: ['$durationMs', 60000] } },
          avgPages: { $avg: '$pagesCount' }
        }
      },
      {
