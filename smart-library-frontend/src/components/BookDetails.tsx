...
const BookDetails: React.FC<BookDetailsProps> = ({ book, currentUser, onBack, onBorrow }) => {
  ...
  const handleSubmitReview = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Append the new review locally without refetching
      const review: Review = {
      ...
      setBookReviews((prev) => [review, ...prev]);
        ...
        <div className="md:flex">
          ...
          <div className="md:w-2/3 lg:w-3/4 p-8">
            ...
            <div className="flex items-center space-x-2 mb-4">
              ...
              <span className="text-gray-600">{book.rating.toFixed(1)} ({book.reviewCount} reviews)</span>
            </div>
            ...
            <div className="bg-gray-50 rounded-xl p-4 mb-6">
              <div className="flex items-center justify-between">
                <p className={`font-medium ${availability.color}`}>
                  {availability.status} - {book.copiesAvailable}/{book.totalCopies} copies available
                </p>
                {currentUser.role === 'user' && book.copiesAvailable > 0 && (
                  <button
                    onClick={() => onBorrow(book)}
                    className="px-6 py-2 bg-blue-700 text-white rounded-lg hover:bg-blue-800 transition-colors duration-200"
                  >
                    Borrow Book
                  </button>
                )}
            ...
            <div className="bg-gray-50 rounded-xl p-6 mb-6">
              <form onSubmit={handleSubmitReview}>
                ...
                <div className="flex space-x-3">
                  <button type="submit" className="px-4 py-2 bg-blue-700 text-white rounded-lg">
                    Submit
                  </button>
                  <button
                    type="button"
