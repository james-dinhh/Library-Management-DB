...
async function updateBookInventory(bookId: number, staffId: number, newTotal: number) {
  const res = await fetch(`${API_BASE}/books/${bookId}/inventory`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ staffId, newTotal })
  });
...
async function retireBook(bookId: number, staffId: number) {
  const res = await fetch(`${API_BASE}/books/${bookId}/retire`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
...
const StaffDashboard: React.FC<StaffDashboardProps> = ({ currentUser }) => {
  const [activeTab, setActiveTab] = useState<'overview' | 'inventory' | 'users'>('inventory');
  ...
  const filteredBooks = books.filter(book => {
    ...
    switch (sortBy) {
      ...
      case 'availability': return b.copiesAvailable - a.copiesAvailable;
      default: return 0;
    }
  });
  ...
  const handleSaveBook = async (bookData: Omit<Book, 'id'>) => {
    try {
      await addBook({ staffId: currentUser.id, ...bookData });
  ...
  const handleDeleteBook = async (bookId: number) => {
    ...
    try {
      await retireBook(bookId, Number(currentUser.id));
      const updatedBooks = await fetchBooks();
      setBooks(updatedBooks);
    } catch (err) {
      console.error(err);
  ...
  const handleInventoryUpdate = async (bookId: number, newTotal: number) => {
    try {
      await updateBookInventory(bookId, Number(currentUser.id), newTotal);
      const updatedBooks = await fetchBooks();
      setBooks(updatedBooks);
    } catch (err) {
      console.error(err);
                    ...
                    {filteredBooks.map(book => (
                      <tr key={book.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          ...
                          }} className="ml-2 text-blue-600">Update</button>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
